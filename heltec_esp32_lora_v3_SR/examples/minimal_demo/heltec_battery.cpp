...
float heltec_battery_voltage_cached(unsigned long interval_ms = 30000) {
    unsigned long now = millis();
    if (now - last_update > interval_ms) {
        last_vbat = heltec_battery_voltage();
        last_update = now;
    }
    return last_vbat;
}
...
int heltec_battery_percent() {
  float v = heltec_battery_voltage_cached();
